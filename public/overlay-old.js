...
// Suivi du dernier état pour détecter les transitions
let lastState = null;
...
function updateUI(data) {
  ...
  if (lastState !== status.state) {
    ...
    if ((lastState === 'printing' || lastState === 'paused') && status.state === 'idle') {
      ...
      const filename = lastFilename || status.filename || 'Impression sans nom';
      ...
      lastFilename = null;
      elements.filename.textContent = 'Aucun';
      elements.progress.textContent = '0%';
      elements.progressFill.style.width = '0%';
      elements.duration.textContent = '00:00:00';
    }
    
    // Si on entre en impression, enregistrer le début
  ...
  lastFilename = status.filename; // Mémoriser le nom complet
  if (displayFilename !== 'Aucun' && status.state !== 'idle') {
  ...
  // Progression
  const progressValue = status.state === 'idle' ? 0 : Math.round(status.progress || 0);
